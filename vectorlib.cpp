...
{
	Vec2<float> v(3.5f, 3.5f);
	auto result = v.magnitude();
	REQUIRE(result == std::sqrtf(24.5f));
}
...
TEST_CASE("Normalize is work", "[vector]")
{
	...
	REQUIRE(result.x == (3.5f / std::sqrtf(24.5f)));
	REQUIRE(result.y == (3.5f / std::sqrtf(24.5f)));
}
...
{
	Vec3<float> v(3.5f, 3.5f, 3.5f);
	auto result = v.magnitude();
	REQUIRE(result == std::sqrtf(36.75f));
}
...
TEST_CASE("Vector3's normalize is work", "[vector]")
{
	Vec3<float> v(3.5f, 3.5f, 3.5f);
	auto result = v.normalize();
	REQUIRE(result.x == (3.5f / std::sqrtf(36.75f)));
	REQUIRE(result.y == (3.5f / std::sqrtf(36.75f)));
	REQUIRE(result.z == (3.5f / std::sqrtf(36.75f)));
}
